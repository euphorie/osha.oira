...
def set_sector_colour_values(self):
    ...
    for brain in brains:
        ...
        if not sector.main_colour:
            ...
            sector.main_foreground_colour = utils.MatchColour(
                sector.main_background_colour, 0.0, 0.6, 0.3
            )
            sector.main_background_bright = utils.IsBright(
                sector.main_background_colour
            )

        if sector.support_colour is None:
            sector.support_colour = support_colour
            sector.support_background_colour = support_colour
            sector.support_foreground_colour = utils.MatchColour(
                sector.support_background_colour
            )
            sector.support_background_bright = utils.IsBright(
                sector.support_background_colour
            )
