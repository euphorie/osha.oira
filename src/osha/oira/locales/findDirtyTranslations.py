...
--output         Specify file to which contents must be written (using =),
...
def get_default(entry):
    ...
    if match:
        ...
        if "Default:" in default:
            print(
                "ERROR! There seems to be a duplicate Default entry for "
                "msgid '%s'" % entry.msgid
            )
    else:
        print(
            "WARNING! msgid '%s' in 'new' file does not have a default "
...
def main():
    ...
    if debug:
        ...
        print(
            "In %s: %d untranslated entries and %d fuzzy entries"
            % (oldfile, len(oldpo.untranslated_entries()), len(oldpo.fuzzy_entries()))
        )
        print(
            "In %s: %d new entries and %d changed entries"
            % (newfile, new_entries, changed_entries)
        )
